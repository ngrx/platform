...
export async function publishToRepo(config: Config) {
  for (let pkg of util.getTopLevelPackages(config)) {
    ...
    console.log(`Deploying @ngrx/${pkg} to ${REPO_URL}`);
...
export async function publishToGitHubPages() {
  ...
  const REPO_URL = `git@github.com:ngrx/platform.git`;
  ...
  console.log(`Deploying Example App to ${REPO_URL}`);

  // await util.git(['push origin gh-pages --force']);
  await process.chdir('../../');
}
