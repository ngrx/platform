...
describe('CodeExampleComponent', () => {
  ...
  it('should set avoidFile class if path has .avoid.', () => {
    const codeExampleComponentElement: HTMLElement = fixture.nativeElement.querySelector(
      'code-example'
    );

    expect(codeExampleComponent.path).toBe('code-path');
    expect(
      codeExampleComponentElement.className.indexOf('avoidFile') === -1
    ).toBe(true);
...
class HostComponent {
  ...
  @ViewChild(CodeExampleComponent, { static: true })
