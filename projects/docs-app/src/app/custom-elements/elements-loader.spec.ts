...
describe('ElementsLoader', () => {
  ...
  describe('loadContainedCustomElements()', () => {
    ...
    beforeEach(() =>
      (loadCustomElementSpy = spyOn(elementsLoader, 'loadCustomElement')));

    it(
      'should attempt to load and register all contained elements',
      fakeAsync(() => {
    ...
    it(
      'should attempt to load and register only contained elements',
      fakeAsync(() => {
        expect(loadCustomElementSpy).not.toHaveBeenCalled();
    ...
    it(
      'should wait for all contained elements to load and register',
      fakeAsync(() => {
      ...
      `;
        ...
        const log: any[] = [];
        elementsLoader
          .loadContainedCustomElements(hostEl)
          .subscribe(
    ...
    it(
      'should fail if any of the contained elements fails to load and register',
      fakeAsync(() => {
        const deferreds = returnPromisesFromSpy(loadCustomElementSpy);
  ...
  describe('loadCustomElement()', () => {
    ...
    it(
      'should be able to load and register an element',
      fakeAsync(() => {
    ...
    it(
      'should wait until the element is defined',
      fakeAsync(() => {
        let state = 'pending';
        elementsLoader
          .loadCustomElement('element-b-selector')
          .then(() => (state = 'resolved'));
        flushMicrotasks();
    ...
    it(
      'should not load and register the same element more than once',
      fakeAsync(() => {
        elementsLoader.loadCustomElement('element-a-selector');
        flushMicrotasks();
        expect(definedSpy).toHaveBeenCalledTimes(1);
        ...
        let state = 'pending';
        elementsLoader
          .loadCustomElement('element-a-selector')
          .then(() => (state = 'resolved'));
        flushMicrotasks();
        expect(state).toBe('resolved');
        expect(definedSpy).not.toHaveBeenCalled();
    ...
    it(
      'should fail if defining the the custom element fails',
      fakeAsync(() => {
        let state = 'pending';
        elementsLoader
          .loadCustomElement('element-b-selector')
          .catch(e => (state = `rejected: ${e}`));
        flushMicrotasks();
        expect(state).toBe('pending');
    ...
    it(
      'should be able to load and register an element again if previous attempt failed',
      fakeAsync(() => {
        elementsLoader.loadCustomElement('element-a-selector');
        flushMicrotasks();
        expect(definedSpy).toHaveBeenCalledTimes(1);
