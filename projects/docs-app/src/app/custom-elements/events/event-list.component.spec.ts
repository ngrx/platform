...
describe('Event List Component', () => {
  ...
  it('should set the upcomingEvents', () => {
    component.upcomingEvents$.subscribe(events =>
      expect(events).toEqual(mockUpcomingEvents)
    );
  });

  it('should set the pastEvents', () => {
    component.pastEvents$.subscribe(events =>
      expect(events).toEqual(mockPastEvents)
    );
  });
});
