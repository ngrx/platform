...
describe('LiveExampleComponent', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      .overrideComponent(EmbeddedStackblitzComponent, {
        set: { template: 'NO IFRAME' },
      });
  ...
  describe('when not embedded', () => {
    ...
    it('should have expected stackblitz & download hrefs when has `name` & `stackblitz`', () => {
      testPath = '/guide/somewhere';
      setHostTemplate(
        '<live-example name="testing" stackblitz="app-specs"></live-example>'
      );
      testComponent(() => {
        const hrefs = getHrefs();
        expect(hrefs[0]).toContain('/testing/app-specs.stackblitz.html');
    ...
    it('should only have a download link when `downloadOnly` attr present', () => {
      setHostTemplate(
        '<live-example downloadOnly>download this</live-example>'
      );
      testComponent(() => {
        const hrefs = getHrefs();
    ...
    it('should add title from <live-example> body', () => {
      ...
      setHostTemplate(
        `<live-example title="ignore this title">${expectedTitle}</live-example>`
      );
      testComponent(() => {
        const anchor = getLiveExampleAnchor();
        expect(anchor.textContent).toBe(expectedTitle, 'anchor content');
  ...
  describe('when embedded', () => {
    ...
    function getEmbeddedStackblitzComponent() {
      const compDe = liveExampleDe.query(
        By.directive(EmbeddedStackblitzComponent)
      );
      return (
        compDe && (compDe.componentInstance as EmbeddedStackblitzComponent)
      );
    }
    ...
    it('should have hidden, embedded stackblitz', () => {
      ...
      testComponent(() => {
        expect(liveExampleComponent.mode).toBe(
          'embedded',
