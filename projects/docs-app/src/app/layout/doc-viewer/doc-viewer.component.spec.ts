...
describe('DocViewerComponent', () => {
  ...
  describe('#doc', () => {
    ...
    it('should render the new document', () => {
      ...
      expect(renderSpy.calls.mostRecent().args).toEqual([
        { id: 'bar', contents: 'foo' },
      ]);
  ...
  describe('#prepareTitleAndToc()', () => {
    ...
    const DOC_WITH_EMBEDDED_TOC =
      '<h1>Features</h1><aio-toc class="embedded"></aio-toc>Some content';
    const DOC_WITH_EMBEDDED_TOC_WITHOUT_H1 =
      '<aio-toc class="embedded"></aio-toc>Some content';
    ...
    describe('(title)', () => {
      it('should set the title if there is an `<h1>` heading', () => {
        ...
        expect(titleService.setTitle).toHaveBeenCalledWith('NgRx - Features');
      });
      ...
      it('should set the default title if there is no `<h1>` heading', () => {
        ...
        expect(titleService.setTitle).toHaveBeenCalledWith('NgRx');
  ...
  describe('#render()', () => {
    ...
    beforeEach(() => {
      ...
      loadElementsSpy = elementsLoader.loadContainedCustomElements.and.returnValue(
        of(undefined)
      );
      prepareTitleAndTocSpy = spyOn(docViewer, 'prepareTitleAndToc');
      swapViewsSpy = spyOn(docViewer, 'swapViews').and.returnValue(
        of(undefined)
      );
    });

    it('should return an `Observable`', () => {
      expect(docViewer.render({ contents: '', id: '' })).toEqual(
    ...
    describe('(contents, title, ToC)', () => {
      ...
      it('should prepare the title and ToC (before embedding components)', async () => {
        prepareTitleAndTocSpy.and.callFake(
          (targetEl: HTMLElement, docId: string) => {
      ...
      it('should set the title and ToC (after the content has been set)', async () => {
        ...
        addTitleAndTocSpy.and.callFake(() =>
          expect(docViewerEl.textContent).toBe('Foo content')
        );
        await doRender('Foo content');
        expect(addTitleAndTocSpy).toHaveBeenCalledTimes(1);

        addTitleAndTocSpy.and.callFake(() =>
          expect(docViewerEl.textContent).toBe('Bar content')
        );
        ...
        addTitleAndTocSpy.and.callFake(() =>
          expect(docViewerEl.textContent).toBe('Qux content')
        );
      ...
      it('should remove the "noindex" meta tag if the document is valid', async () => {
        ...
        expect(TestBed.get(Meta).removeTag).toHaveBeenCalledWith(
          'name="robots"'
        );
      });
      ...
      it('should add the "noindex" meta tag if the document is 404', async () => {
        ...
        expect(TestBed.get(Meta).addTag).toHaveBeenCalledWith({
          name: 'robots',
          content: 'noindex',
        });
      });
    ...
    describe('(embedding components)', () => {
      ...
      it('should attempt to embed components even if the document is empty', async () => {
        ...
        expect(loadElementsSpy.calls.argsFor(0)).toEqual([
          docViewer.nextViewContainer,
        ]);
        expect(loadElementsSpy.calls.argsFor(1)).toEqual([
          docViewer.nextViewContainer,
        ]);
      });
      ...
      it('should unsubscribe from the previous "embed" observable when unsubscribed from', () => {
        ...
        const renderObservable = docViewer.render({
            ...
            it('should emit `docRemoved` after removing the leaving view', async () => {
              const onDocRemovedSpy = jasmine
                ...
                .and.callFake(() => {
                  ...
                  expect(docViewerEl.contains(oldNextViewContainer)).toBe(
                    false
                  );
                });
