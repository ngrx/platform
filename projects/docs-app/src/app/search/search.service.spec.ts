...
describe('SearchService', () => {
  ...
  describe('initWorker', () => {
    it(
      'should create the worker and load the index after the specified delay',
      fakeAsync(() => {
        ...
        tick(100);
        expect(WebWorkerClient.create).toHaveBeenCalledWith(
          'some/url',
          jasmine.any(NgZone)
        );
  ...
  describe('search', () => {
    ...
    it('should push the response to the returned observable', () => {
      ...
      (mockWorker.sendMessage as jasmine.Spy).and.returnValue(
        of(mockSearchResults)
      );
      service
        .search('some query')
        .subscribe(results => (actualSearchResults = results));
