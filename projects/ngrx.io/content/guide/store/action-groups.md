...
The `emptyProps` function is used to define an action without payload within an action group.
...
Fortunately, this is not the case with the `createActionGroup` function - we will get a compilation error if two actions from the same group have the same type.
...
This makes it easier to search for their usages within the codebase.
