...
```ts
// Callback triggered by an event not running within NgZone
function callbackOutsideNgZone(){
  this.store.dispatch(clearTodos());
}
```

The fix for this violation is to ensure actions are running within `NgZone`. Identify the event trigger and verify if the code can be updated to use an `NgZone` aware Angular feature, i.e `HostListener` for events. If this is not possible use `NgZone.run` to explicitly make Angular aware of this asynchronous task.
