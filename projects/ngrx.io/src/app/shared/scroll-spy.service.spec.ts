...
describe('ScrollSpyService', () => {
    ...
    describe('#spyOn()', () => {
        ...
        it('should initialize the newly created `ScrollSpiedElementGroup`', () => {
            const calibrateSpy = spyOn(
                ScrollSpiedElementGroup.prototype,
                'calibrate'
            );
        ...
        it('should call `onResize()` if it is the first `ScrollSpiedElementGroup`', () => {
            ...
            const onResizeSpy = spyOn(
                ScrollSpyService.prototype as any,
                'onResize'
    ...
    describe('window resize events', () => {
        ...
        it(
            'should be subscribed to when the first group of elements is spied on',
            fakeAsync(() => {
        ...
        it(
            'should be unsubscribed from when the last group of elements is removed',
            fakeAsync(() => {
                const info1 = scrollSpyService.spyOn([]);
                const info2 = scrollSpyService.spyOn([]);
                onResizeSpy.calls.reset();
        ...
        it(
            `should only fire every ${RESIZE_EVENT_DELAY}ms`,
            fakeAsync(() => {
                scrollSpyService.spyOn([]);
                onResizeSpy.calls.reset();
    ...
    describe('window scroll events', () => {
        ...
        it(
            `should only fire every ${SCROLL_EVENT_DELAY}ms`,
            fakeAsync(() => {
                scrollSpyService.spyOn([]);
    ...
    describe('#onResize()', () => {
        it('should re-calibrate each `ScrollSpiedElementGroup`', () => {
            ...
            const spiedElemGroups: ScrollSpiedElementGroup[] = (scrollSpyService as any)
                .spiedElementGroups;
            const calibrateSpies = spiedElemGroups.map(group =>
                spyOn(group, 'calibrate')
            );
    ...
    describe('#onScroll()', () => {
        it('should propagate to each `ScrollSpiedElementGroup`', () => {
            ...
            const spiedElemGroups: ScrollSpiedElementGroup[] = (scrollSpyService as any)
                .spiedElementGroups;
            const onScrollSpies = spiedElemGroups.map(group =>
                spyOn(group, 'onScroll')
            );
        ...
        it('should first re-calibrate if the content height has changed', () => {
            ...
            const calibrateSpies = spiedElemGroups.map((group, i) =>
                spyOn(group, 'calibrate').and.callFake(() =>
                    expect(onScrollSpies[i]).not.toHaveBeenCalled()
                )
            );
