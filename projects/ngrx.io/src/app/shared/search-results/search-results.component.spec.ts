...
describe('SearchResultsComponent', () => {
  ...
  it('should put first 5 results for each area into priorityPages', () => {
    ...
    expect(component.searchAreas[0].priorityPages).toEqual(
      results.filter(p => p.path.startsWith('api')).slice(0, 5)
    );
    expect(component.searchAreas[1].priorityPages).toEqual(
      results.filter(p => p.path.startsWith('guide')).slice(0, 5)
    );
  });
