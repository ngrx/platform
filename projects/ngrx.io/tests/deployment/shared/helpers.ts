...
import {
  FirebaseRedirector,
  FirebaseRedirectConfig,
} from '../../../tools/firebase-test-utils/FirebaseRedirector';
...
function extractSitemapUrls(xml: string) {
  ...
  xml.replace(
    /<loc>([^<]+)<\/loc>/g,
    (_, loc) => urls.push(loc.replace(host, '')) as any
  );
  ...
  urls.forEach(url => {
    if (url.includes('://')) {
      throw new Error(
        `Sitemap URL (${url}) contains unexpected host. Expected: ${host}`
      );
    }
  });
