...
export class SitePage {
  ...
  ghLinks = this.docViewer
    .all(by.css('a'))
    .filter((a: ElementFinder) =>
      a.getAttribute('href').then(href => githubRegex.test(href))
    );
  ...
  getNavItem(pattern: RegExp) {
    return element
      ...
      .filter(elementFinder =>
        elementFinder.getText().then(text => pattern.test(text))
      )
      .first();
  }
  ...
  ga() {
    return browser.executeScript('return window["ga"].q') as promise.Promise<
      any[][]
    >;
  }
  locationPath() {
  ...
  navigateTo(pageUrl) {
    ...
    return browser
      .get('/' + pageUrl)
      .then(() =>
        browser.executeScript("document.body.classList.add('no-animations')")
      )
