...
export class FirebaseRedirect {
  ...
  replace(url: string) {
    ...
    if (match) {
      const paramReplacers = Object.keys(this.glob.namedParams).map(name => [
        XRegExp(`:${name}`, 'g'),
        match[name],
      ]);
      const restReplacers = Object.keys(this.glob.restParams).map(name => [
        XRegExp(`:${name}\\*`, 'g'),
        match[name],
      ]);
      return XRegExp.replaceEach(this.destination, [
        ...paramReplacers,
        ...restReplacers,
      ]);
    }
