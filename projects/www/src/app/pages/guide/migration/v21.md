...
When composing a parameterized selector with other selectors, you can return a lookup function from the projector. This keeps the parent selector memoized — it only recomputes when its input selectors change, not when the parameter changes.
